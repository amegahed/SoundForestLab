...
export default PreferencesFormView.extend({
	...
	template: template(`
		...
		<div class="window-panes form-group">
			...
			<div class="controls">
				...
				<div class="show-analyser checkbox-inline">
					<label><input type="checkbox"<% if (show_analyser) { %> checked<% } %>>Analyser</label>
				</div>
		...
		<div class="sidebar-panels form-group">
			...
			<div class="controls">
				...
				<div class="checkbox-inline">
					<label><input type="checkbox" value="track_info"<% if (sidebar_panels.includes('track_info')) { %> checked<% } %>>Track Info</label>
				</div>
		...
		<div class="analyzer-size form-group">
			<label class="control-label"><i class="fa fa-arrows-alt-v"></i>Analyser Size (%)</label>
			<div class="controls">
				<div class="range-input"></div>
		
				<div class="control-inline">
					<i class="active fa fa-question-circle" data-toggle="popover" title="Analyser Size" data-content="This is the size of the analyzer window pane."></i>
				</div>
			</div>
		</div>
	`),
	...
	regions: {
		...
		analyzer_size: '.analyzer-size .range-input'
	},
	...
	events: {
		...
		'change .show-analyser input': 'onChangeShowAnalyser'
	},
	...
	getValue: function(key) {
		switch (key) {
			...
			case 'show_analyser':
				return this.$el.find('.show-analyser input').is(':checked');
			case 'analyzer_size':
				return this.getChildView('analyzer_size').getValue();
		}
	},
	...
	getValues: function() {
		return {
			...
			show_analyser: this.getValue('show_analyser'),
			analyzer_size: this.getValue('analyzer_size'),
		};
	},
	...
	setValue: function(key, value) {
		switch (key) {
			...
			case 'sidebar_panels':
				...
				break;
			case 'show_analyser':
				this.$el.find('.show-analyser input[type="checkbox"]').prop('checked', value);
				break;
			case 'analyzer_size':
				this.$el.find('.analyzer_size input[type="checkbox"]').prop('checked', value);
				break;
		}
	},
	...
	showAnalyzerSize: function() {
		this.showChildView('analyzer_size', new RangeInputView({

			// options
			//
			value: this.model.get('analyzer_size'),
			min: 0,
			max: 100,
			step: 5,
			...
			// callbacks
			//
			onchange: () => this.onChangeAnalyzerSize()
		}));
	},
	...
	onChangeShowAnalyser: function() {
		this.onChangeValue('show_analyser', this.getValue('show_analyser'));
	},

	onChangeAnalyzerSize: function() {
		this.onChangeValue('analyzer_size', this.getValue('analyzer_size'));
	}
});

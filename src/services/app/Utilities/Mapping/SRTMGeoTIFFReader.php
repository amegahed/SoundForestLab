...
|******************************************************************************|
|                                                                              |
|        This defines a utility for parsing geotiff metadata.                  |
|                                                                              |
...
namespace App\Utilities\Mapping;
...
class SRTMGeoTIFFReader {
    ...
    const NUM_DATA_ROWS = 6001;     // the number of data rows in the file ( = ImageLength tag value)
    const NUM_DATA_COLS = 6001;     // the number of data columns in the file ( = ImageWidth tag value)
    ...
    const STRIPOFFSETS = 0x44c1b2a; // the offset address of the 'StripOffsets' tag
	const NO_DATA = 0;
    ...
    */
    public function getMultipleElevations($latLons, $addIntermediatelatLons = true, $interpolate = false) {
        ...
        if ($numlatLons < 4) {
			...
			return $this->elevations = array($this->getElevation($latLons[0], $latLons[1], $interpolate));
        }
        ...
        if ($addIntermediatelatLons) {
            ...
            for ($i = 2; $i < $numlatLons; $i += 2) {
	            ...
	            $elevations[] = $this->getElevation($startLat, $startLon, $interpolate);
    ...
    */
    private function getInterpolatedElevation($latitude, $longitude) {
        ...
        $col[0] = floor(abs(abs($this->topleftLon) - abs($longitude)) / self::DEGREES_PER_TILE * (self::NUM_DATA_COLS -1));
    ...
    * @return float
    */
    public function getDistance($lat1, $lon1, $lat2, $lon2, $miles=true){
